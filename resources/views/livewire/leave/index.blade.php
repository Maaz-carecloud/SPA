            ...
            foreach($leaves as $index => $leave) {
                $rows[] = [
                    ...
                    '<div class="action-items">' .
                        '<span><a @click.prevent="$dispatch(\'edit-mode\', {id: ' . $leave->id . '})" data-bs-toggle="modal" data-bs-target="#createModal"><i class="fa fa-edit"></i></a></span>' .
                        '<span><a href="javascript:void(0)" class="delete-swal" data-id="' . $leave->id . '"><i class="fa fa-trash"></i></a></span>' .
                    '</div>'
                ];
            }
        }
        ...
        <form enctype="multipart/form-data">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select wire:model.lazy="user_id" class="form-select @error('user_id') is-invalid @enderror" id="user_id" @if(!$user_type) disabled @endif>
                        <option value="">
                            @if($user_type)
                    ...
                    @if($attachment)
                        <div class="mt-2">
                            <div class="alert alert-success d-flex align-items-center">
                                ...
                                <div>
                                    <strong>File Selected:</strong> {{ is_object($attachment) ? $attachment->getClientOriginalName() : $attachment }}
                                    @if(is_object($attachment))<br><small>Size: {{ number_format($attachment->getSize() / 1024, 2) }} KB</small>@endif
                                </div>
                            </div>
                        </div>
                    @endif
